...
struct global_args {
    ...
    short int velocities[6];
    short int accelerations[6];
    short int measurement_offset[6];
...
uint8_t resolution[6];         // sensors resolution set on the board
...
//=========================================================     get measurements
    ...
    if(global_args.flag_get_measurements)
    {
        ...
        while(1) {
            sensor_num = commGetExoSenseMeasurements(&comm_settings_1, global_args.device_id, global_args.measurements);

            if(sensor_num < 0 || sensor_num > 6) {
...
//===============================================================     input file
    ...
    if(global_args.flag_set_zeros)
    {
        ...
        while(1) {
            ...
            commGetExoSenseMeasurements(&comm_settings_1, global_args.device_id,
                    global_args.measurements);
            for (i = 0; i < sensor_num; i++) {
...
void int_handler_2(int sig) {
    int i;
    short int temp_meas[6];
    int sensor_num  = 0;

    sensor_num = commGetExoSenseMeasurements(&comm_settings_1, global_args.device_id, temp_meas);
