...
% LIBRARY UPDATE
display('[INFO] ExoSense library should be installed after QB Move library');
...
old_file = strcat('', lib);
old_file = strcat(old_file, 'vers/ExoSense_library_');
old_file = strcat(old_file, ver);

new_file = strcat('', lib);
new_file = strcat(new_file, 'ExoSense_library');
...
if ( (str2double(ver(1:end-1)) >= 2015) )
    ...
    if ~exist(old_file, 'file')
        ...
        old_file = strcat('library/vers/ExoSense_library_', num2str(last_version));
...
display('[INFO] ExoSense library install successful');
